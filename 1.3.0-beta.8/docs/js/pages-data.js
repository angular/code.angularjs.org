...
angular.module('pagesData', [])
  ...
  "api/ng/type/ngModel.NgModelController": {
    ...
    "searchTerms": {
      ...
      "keywords": "$dirty $error $formatters $invalid $modelvalue $parsers $pristine $setvalidity $valid $viewchangelisteners $viewvalue $watches achieve actual additional api arguments array attribute bound browser browsers called changed changes collaborate contenteditable contents control controller convert css custom data-binding deals desired directive directives display dom edited element error errors events example example-ngmodelcontroller execute format formatter formatting function functions hash html5 ignored input interacted invalid js js-angular-release listening logic model ng ng-model ngmodel ngmodelcontroller note notice object older parsers parsing passing pipeline place populate provided purposefully push reads rendering required result return sanitize services src string tells touppercase true turn type undefined update updates user validation validity values view work"
    }
  },
  ...
  "api/ng/directive/ngModelOptions": {
    ...
    "searchTerms": {
      ...
      "keywords": "$rollbackviewvalue access action actual allows api apply attribute belonging blur bound button called canceled change changes clear control controllers current custom debounce debounced debouncing default delay delimited directive displayed easiest empty enclosing escape event events example example-ngmodeloptions-directive-blur example-ngmodeloptions-directive-debounce expires field fields focus focused form input inputs inside integer invoke js js-angular-release key list loses making matches method milliseconds model nature ng ngclick ngmodel ngmodelcontroller ngmodeloptions ngmodeloptionsdirective ngsubmit note object occur options order override pending place pressed published reference relevant reset scope set space special src string submit submitted supplied synchronized takes timer trigger triggers tuning update updated updateon updates valid view"
    }
  },
  ...
  "api/ng/directive/ngStyle": {
    ...
    "searchTerms": {
      ...
      "keywords": "ac allows api conditionally corresponding css directive element evals example-example47 expression guide html js js-angular-release names ng ngstyle ngstyledirective object set src style values"
    }
  },
  ...
  "api/ng/service/$filter": {
    ...
    "searchTerms": {
      "titleWords": "$filter",
      "keywords": "$filter $inject api data displayed expression filter filter_name filters formatting function general js js-angular-release ng retrieve service src syntax templates user"
    }
  },
  ...
  "api/ng/service/$interpolate": {
    ...
    "searchTerms": {
      ...
      "keywords": "$compile $interpolate $interpolateprovider $parse $sce allornothing angular api argument behavior binding compiles compute configuring context contextual data default details embedded escaping evaluate evaluation exp expect expression expressions false forgiving fourth function gettrusted hello html injected interpolate interpolated interpolating interpolation js js-angular-release markup mode musthaveexpression ng ngsrc ngsrcset null optional order parameters passes provided refer result return returned returning returns service set src strict string strings takes text tobeundefined toequal true trustedcontext undefined uppercase urls var"
    }
  },
  ...
  "api/ngSanitize/filter/linky": {
    ...
    "searchTerms": {
      "titleWords": "linky",
      "keywords": "address api email example-example66 filter finds frame html html-linkified http input installed js js-angular-release links linky linky_expression module named ng-bind-html ngsanitize open plain requires src supports target text turns window"
    }
  },
  "api/ngSanitize": {
    "docType": "module",
  ...
  "guide/expression": {
    ...
    "searchTerms": {
      ...
      "keywords": "$eval $event $parse $rootscope $window access accidental alert angular application binding bindings bugs call clickme clutter code common complex conditional conditionals context control controller controllers core data default defined delegate differences directive directives displaying docs eval evaluate evaluated evaluating evaluation evaluations example example-example92 example-example93 example-example94 example-example95 exception exceptions explicitly expose expression expressions filters flow forgiving format function general generates global guide intentional invoking items javascript javascript-like js-angular-release language logic loop loops method names ng ngclick ngdoc ngfocus note null object overview pass philosophy prevents processes properties purpose reason refer referenceerror response restriction returns scope sense server service simply snippets source statement statements subtle throw throws typeerror undefined user valid view views waiting wasn window write"
    }
  },
  "guide/filter": {
    "docType": "overview",
